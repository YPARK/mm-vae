...
int
reheader_file(const char *header, const char *file, int meta)
{
    ...
    if (fp->block_length - skip_until > 0) {
        if (bgzf_write(bgzf_out,
                       buffer + skip_until,
                       fp->block_length - skip_until) < 0)
            error("Error: %d\n", fp->errcode);
    }
...
main(int argc, char *argv[])
{
    int c, skip = -1, meta = -1, list_chrms = 0, force = 0, print_header = 0,
           print_only_header = 0, bed_reg = 0, bed_comp = 0;
    ...
    while ((c = getopt(argc, argv, "p:s:b:e:0S:c:lhHfCBr:")) >= 0) {
        ...
        case 'p':
            ...
            else if (strcmp(optarg, "bed") == 0)
                conf_ptr = &ti_conf_bed;
            else if (strcmp(optarg, "sam") == 0)
                conf_ptr = &ti_conf_sam;
            else if (strcmp(optarg, "vcf") == 0 || strcmp(optarg, "vcf4") == 0)
                conf_ptr = &ti_conf_vcf;
            else if (strcmp(optarg, "psltbl") == 0)
                conf_ptr = &ti_conf_psltbl;
            else {
    ...
    if (!conf_ptr) {
        ...
        if (l >= 7 && strcasecmp(argv[optind] + l - 7, ".gff.gz") == 0)
            conf_ptr = &ti_conf_gff;
        else if (l >= 7 && strcasecmp(argv[optind] + l - 7, ".bed.gz") == 0)
            conf_ptr = &ti_conf_bed;
        else if (l >= 7 && strcasecmp(argv[optind] + l - 7, ".sam.gz") == 0)
            conf_ptr = &ti_conf_sam;
        else if (l >= 7 && strcasecmp(argv[optind] + l - 7, ".vcf.gz") == 0)
            conf_ptr = &ti_conf_vcf;
        else if (l >= 10 &&
                 strcasecmp(argv[optind] + l - 10, ".psltbl.gz") == 0)
            conf_ptr = &ti_conf_psltbl;
    }
    if (conf_ptr)
        conf = *conf_ptr;
    ...
    if (optind + 1 == argc && !print_only_header) {
        if (force == 0) {
            if (stat(fnidx, &stat_tbi) == 0) {
                // Before complaining, check if the VCF file isn't newer. This
        ...
        if (bgzf_is_bgzf(argv[optind]) != 1) {
            fprintf(stderr,
                    "[tabix] was bgzip used to compress this file? %s\n",
                    argv[optind]);
            free(fnidx);
            return 1;
        }
        if (!conf_ptr) {
            // Building the index but the file type was neither recognised nor
            // given. If no custom change
            //  has been made, warn the user that GFF is used
            if (conf.preset == ti_conf_gff.preset &&
                conf.sc == ti_conf_gff.sc && conf.bc == ti_conf_gff.bc &&
    ...
    { // retrieve
        tabix_t *t;
        // On some systems, stat on non-existent files returns undefined value
        // for sm_mtime, the user had to use -f
        int is_remote = (strstr(fnidx, "ftp://") == fnidx ||
        ...
        if (strcmp(argv[optind + 1], ".") == 0) { // retrieve all
            ...
            if (bed_reg) {
                extern int bed_overlap(const void *_h,
                                       const char *chr,
                                       int beg,
                                       int end);
                ...
                const ti_conf_t *conf_ = idxconf ?
                    idxconf :
                    &conf; // use the index file if available
                ...
                while ((s = ti_read(t, iter, &len)) != 0) {
                    ...
                    *intv.se = '\0';
                    ...
                    if ((!bed_comp && is_ovlp) ||
                        (bed_comp && !is_ovlp)) { // or (bed_comp ^ is_ovlp)
                        *intv.se = c;
                        puts(s);
                    }
                    *intv.se = c;
                ...
                for (i = optind + 1; i < argc; ++i) {
                    int tid, beg, end;
                    if (ti_parse_region(t->idx, argv[i], &tid, &beg, &end) ==
                        0) {
                    ...
                    // else fprintf(stderr, "[main] invalid region: unknown
                    // target name or minus interval.\n");
                }
            }
        }
        ti_close(t);
