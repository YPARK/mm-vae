 ...
 * commented out above. */
int
ti_readline(BGZF *fp, kstring_t *str)
{
...
static inline int
ti_reg2bin(uint32_t beg, uint32_t end)
{
    --end;
    if (beg >> 14 == end >> 14)
        return 4681 + (beg >> 14);
...
static int
get_tid(ti_index_t *idx, const char *ss)
{
    khint_t k;
    int tid;
...
ti_get_intv(const ti_conf_t *conf, int len, char *line, ti_interval_t *intv)
{
        ...
        if (line[i] == '\t' || line[i] == 0) {
            ...
            if (id == conf->sc) {
                ...
                if ((conf->preset & 0xffff) == TI_PRESET_GENERIC) {
                    ...
                    // FIXME: the following is NOT tested and is likely to be
                    // buggy
                    if (id == 4) {
                        if (b < i)
                            intv->end = intv->beg + (i - b);
    ...
    /*
      ...
      if (ncols == 1) fprintf(stderr,"[get_intv] Is the file tab-delimited? The
      line has %d field only: %s\n", ncols, line); else
      fprintf(stderr,"[get_intv] The line has %d field(s) only: %s\n", ncols,
      line); exit(1);
      }
    */
    if (intv->ss == 0 || intv->se == 0 || intv->beg < 0 || intv->end < 0)
        return -1;
    return 0;
}

static int
get_intv(ti_index_t *idx, kstring_t *str, ti_intv_t *intv)
{
    ti_interval_t x;
    ...
    if (ti_get_intv(&idx->conf, str->l, str->s, &x) == 0) {
        ...
        *x.se = c;
        ...
        fprintf(stderr,
                "[%s] the following line cannot be parsed and skipped: %s\n",
                __func__,
                str->s);
        return -1;
    }
...
static inline void
insert_offset(khash_t(i) * h, int bin, uint64_t beg, uint64_t end)
{
    khint_t k;
    ti_binlist_t *l;
    int ret;
...
insert_offset2(ti_lidx_t *index2, int _beg, int _end, uint64_t offset)
{
    int i, beg, end;
...
ti_index_t *
ti_index_core(BGZF *fp, const ti_conf_t *conf)
{
    int ret;
    ti_index_t *idx;
    ...
    save_off = last_off = bgzf_tell(fp);
    ...
    while ((ret = ti_readline(fp, str)) >= 0) {
        ...
        if (last_tid != intv.tid) { // change of chromosomes
            if (last_tid > intv.tid) {
                fprintf(
                    stderr,
                    "[ti_index_core] the chromosome blocks not continuous at line %llu, is the file sorted? [pos %d]\n",
            ...
            fprintf(stderr,
                    "[ti_index_core] the file out of order at line %llu\n",
                    (unsigned long long)lineno);
            exit(1);
        }
        tmp = insert_offset2(&idx->index2[intv.tid],
                             intv.beg,
                             intv.end,
                             last_off);
        if (last_off == 0)
        ...
        if (intv.bin != last_bin) { // then possibly write the binning index
            if (save_bin != 0xffffffffu) // save_bin==0xffffffffu only happens
                                         // to the first record
                insert_offset(idx->index[save_tid],
                              save_bin,
                              save_off,
                              last_off);
            save_off = last_off;
        ...
        if (bgzf_tell(fp) <= last_off) {
            fprintf(stderr,
                    "[ti_index_core] bug in BGZF: %llx < %llx\n",
                    (unsigned long long)bgzf_tell(fp),
...
void
ti_index_destroy(ti_index_t *idx)
{
    khint_t k;
    int i;
    if (idx == 0)
...
void
ti_index_save(const ti_index_t *idx, BGZF *fp)
{
    ...
    if (ti_is_be) { // write ti_conf_t;
        ...
        for (i = 0; i < 6; ++i)
            bgzf_write(fp, bam_swap_endian_4p(&x[i]), 4);
    } else
    ...
    { // write target names
        ...
        for (i = 0; i < kh_size(idx->tname); ++i)
            l += strlen(name[i]) + 1;
        if (ti_is_be)
            bgzf_write(fp, bam_swap_endian_4p(&l), 4);
        else
            bgzf_write(fp, &l, 4);
        for (i = 0; i < kh_size(idx->tname); ++i)
...
ti_index_t *
ti_index_load_local(const char *fnidx)
{
    BGZF *fp;
...
#ifdef _USE_KNETFILE
static void
download_from_remote(const char *url)
{
    ...
    knetFile *fp_remote;
    int l;
    if (strstr(url, "ftp://") != url && strstr(url, "http://") != url)
        return;
    l = strlen(url);
    ...
    if ((fp = fopen(fn, "w")) == 0) {
        fprintf(
            stderr,
            "[download_from_remote] fail to create file in the working directory.\n");
...
static char *
get_local_version(const char *fn)
{
    struct stat sbuf;
    char *fnidx = (char *)calloc(strlen(fn) + 5, 1);
    strcat(strcpy(fnidx, fn), ".tbi");
    if ((strstr(fnidx, "ftp://") == fnidx ||
         strstr(fnidx, "http://") == fnidx)) {
        char *p, *url;
        int l = strlen(fnidx);
...
const char **
ti_seqname(const ti_index_t *idx, int *n)
{
    const char **names;
    khint_t k;
    *n = idx->n;
...
ti_index_load(const char *fn)
{
    ...
    if (idx == 0)
        fprintf(stderr, "[ti_index_load] fail to load the index: %s\n", fname);
    free(fname);
    return idx;
}

int
ti_index_build2(const char *fn, const ti_conf_t *conf, const char *_fnidx)
{
    char *fnidx;
    BGZF *fp, *fpidx;
    ti_index_t *idx;
...
int
ti_index_build(const char *fn, const ti_conf_t *conf)
{
...
int
ti_get_tid(const ti_index_t *idx, const char *name)
{
    khiter_t iter;
...
int
ti_parse_region(const ti_index_t *idx,
                const char *str,
                int *tid,
                int *begin,
                int *end)
...
static inline int
reg2bins(uint32_t beg, uint32_t end, uint16_t list[MAX_BIN])
{
    int i = 0, k;
    if (beg >= end)
        return 0;
...
ti_iter_t
ti_iter_query(const ti_index_t *idx, int tid, int beg, int end)
{
    uint16_t *bins;
    int i, n_bins, n_off;
    pair64_t *off;
    ...
    if (idx->index2[tid].n > 0) {
        ...
        if (min_off ==
            0) { // improvement for index files built by tabix prior to 0.1.4
...
const char *
ti_iter_read(BGZF *fp, ti_iter_t iter, int *len)
{
    if (iter->finished)
        return 0;
    if (iter->from_first) {
    ...
    while (1) {
        ...
        if (iter->curr_off == 0 ||
            iter->curr_off >=
                iter->off[iter->i].v) { // then jump to the next chunk
            ...
            if (iter->i < 0 ||
                iter->off[iter->i].v !=
                    iter->off[iter->i + 1]
                        .u) { // not adjacent chunks; then seek
        ...
        if ((ret = ti_readline(fp, &iter->str)) >= 0) {
            ti_intv_t intv;
            iter->curr_off = bgzf_tell(fp);
            if (iter->str.s[0] == iter->idx->conf.meta_char)
                continue;
            get_intv((ti_index_t *)iter->idx, &iter->str, &intv);
            if (intv.tid != iter->tid || intv.beg >= iter->end)
...
void
ti_iter_destroy(ti_iter_t iter)
{
    if (iter) {
        free(iter->str.s);
        free(iter->off);
...
const ti_conf_t *
ti_get_conf(ti_index_t *idx)
{
    return idx ? &idx->conf : 0;
}
...
tabix_t *
ti_open(const char *fn, const char *fnidx)
{
    tabix_t *t;
    BGZF *fp;
    if ((fp = bgzf_open(fn, "r")) == 0)
...
ti_lazy_index_load(tabix_t *t)
{
    if (t->idx == 0) { // load index
        ...
        if (t->idx == 0)
            return -1; // fail to load index
    }
    return 0;
}

ti_iter_t
ti_queryi(tabix_t *t, int tid, int beg, int end)
{
    if (tid < 0)
...
ti_iter_t
ti_querys(tabix_t *t, const char *reg)
{
    int tid, beg, end;
    if (reg == 0)
        return ti_iter_first();
    if (ti_lazy_index_load(t) != 0)
        return 0;
    if (ti_parse_region(t->idx, reg, &tid, &beg, &end) < 0)
        return 0;
    return ti_iter_query(t->idx, tid, beg, end);
}

ti_iter_t
ti_query(tabix_t *t, const char *name, int beg, int end)
{
    int tid;
    if (name == 0)
        return ti_iter_first();
...
const char *
ti_read(tabix_t *t, ti_iter_t iter, int *len)
{
