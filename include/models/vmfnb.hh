...
DIM(data_dim);
DIM(mu_shared_enc_h_dim);
DIM(mu_nb_dec_h_dim);
DIM(mu_vmf_dec_h_dim);
DIM(mu_shared_enc_r_dim);
...
struct vmfnb_options_t {
    ...
    std::vector<int64_t> vmf_decoding_layers;
    int64_t mean_latent;
                    ...
                    vmfnb_options_t &options)
{
    const char *_usage =
        ...
        "--vmf_decoding      : dims for vMF decoding layers (e.g., 10,10)\n"
    ...
    const char *const short_opts = "E:D:V:L:k:K:Rrh";
    ...
    const option long_opts[] = {
        ...
        { "vmf_decoding", required_argument, nullptr, 'V' },            //
        { "vmf-decoding", required_argument, nullptr, 'V' },            //
    ...
    while (true) {
        ...
        case 'V':
            copy_int_arr(std::string(optarg), options.vmf_decoding_layers);
            break;
...
struct vmfnb_vae_tImpl : torch::nn::Module {
    ...
    vmfnb_vae_tImpl(const data_dim,
                    const std::vector<mu_shared_enc_h_dim>,
                    const std::vector<mu_nb_dec_h_dim>,
                    const mu_shared_enc_r_dim,
                    const nu_nb_enc_h_dim,
                    const nu_nb_enc_r_dim,
                    const std::vector<mu_vmf_dec_h_dim>,
    ...
    std::pair<torch::Tensor, torch::Tensor> shared_encode_mu(torch::Tensor x);
    torch::Tensor nb_decode_mu(torch::Tensor z);
    torch::Tensor vmf_decode_mu(torch::Tensor z);
    ...
    torch::nn::Linear nb_mu_repr_mean { nullptr };
    ...
    torch::nn::Sequential vmf_mu_dec;       // hidden -> vMF
...
vmfnb_vae_tImpl::vmfnb_vae_tImpl(
    const data_dim d_,
    const std::vector<mu_shared_enc_h_dim> mu_nb_enc,
    const std::vector<mu_nb_dec_h_dim> mu_nb_dec,
    const mu_shared_enc_r_dim mu_shared_repr,
    const nu_nb_enc_h_dim nu_nb_enc,
    const nu_nb_enc_r_dim nu_nb_repr,
    const std::vector<mu_vmf_dec_h_dim> mu_vmf_dec,
    ...
    , do_relu(_do_relu)
{
    ...
    if (mu_nb_enc.size() < 1) {
        nb_mu_enc->push_back(torch::nn::Linear(d_prev, mu_shared_repr.val));
        if (do_relu)
            nb_mu_enc->push_back(torch::nn::ReLU(mu_shared_repr.val));

        d_prev = mu_shared_repr.val;
    }

    nb_mu_repr_mean =
        register_module("[shared] mu: representation of the mean",
                        torch::nn::Linear(d_prev, mu_shared_repr.val));

    nb_mu_repr_lnvar =
        register_module("[shared] mu: representation log variance",
                        torch::nn::Linear(d_prev, mu_shared_repr.val));
    ...
    // Add hidden layers between representation and x
    d_prev = mu_shared_repr.val;
    for (int l = 0; l < mu_vmf_dec.size(); ++l) {
        int64_t d_next = mu_vmf_dec[l].val;
        const std::string _str = "mu: decoding " + std::to_string(l + 1);
        vmf_mu_dec->push_back(torch::nn::Linear(d_prev, d_next));
        if (do_relu)
            vmf_mu_dec->push_back(torch::nn::ReLU(d_next));
        d_prev = d_next;
    }

    // Add final one mapping to the reconstruction
    vmf_mu_dec->push_back(torch::nn::Linear(d_prev, D));
}

std::pair<torch::Tensor, torch::Tensor>
vmfnb_vae_tImpl::shared_encode_mu(torch::Tensor x)
{
    ...
    auto mu = nb_mu_repr_mean->forward(h);
...
torch::Tensor
vmfnb_vae_tImpl::vmf_decode_mu(torch::Tensor z)
{
    namespace F = torch::nn::functional;
    auto h = vmf_mu_dec->forward(z);
...
vmfnb_vae_tImpl::forward(torch::Tensor x)
{
    ...
    auto shared_mu_enc_out = shared_encode_mu(x);
    auto shared_mu_mean = shared_mu_enc_out.first;
    auto shared_mu_lnvar = shared_mu_enc_out.second;

    /////////////////////////////
    // Negative binomial model //
    /////////////////////////////

    auto nb_mu_ = nb_decode_mu(reparameterize(shared_mu_mean, shared_mu_lnvar));
    ...
    auto vmf_recon =
        vmf_decode_mu(reparameterize(shared_mu_mean, shared_mu_lnvar));
...
vmfnb_vae_tImpl::normalize_vmf_x(torch::Tensor x)
{
    ...
    return F::normalize(x.log1p() + eps, opt_);
}
...
struct vmfnb_recorder_t {
    ...
    void update_on_batch(MODEL &model, DB &db, BAT &batches)
    {
        ...
        auto mu_out = model->shared_encode_mu(x);
...
struct composite_loss_t {
    ...
    torch::Tensor operator()(X x, Y y, const int64_t epoch)
    {
        ...
        return (nb_loss(x, y) + vmf_loss(x, y) + rate * kl_nb) / n;
    }
